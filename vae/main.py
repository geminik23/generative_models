...
def main():
    ...
    parser.add_argument('--model', type=str, default='mlp', choices=['mlp', 'unet'], help='Select the model to use (default: mlp)')
    ...
    # Dataset
    dataset = get_digit_dataset(IMG_SIZE, None, flatten=not(is_2d))
    ...
    if args.model == 'mlp':
        LEARNING_RATE = 1e-3
        model = VAE(DIM, HIDDEN_DIM, LATENT_DIM).to(device)
    ...
    if args.mode == 'train':
        ...
        torch.save(results, os.path.join(result_dir, get_filename_with_timestamp(f"{MODEL}_{args.model}")))
        ...
        plot_imgs(imgs, IMG_SIZE, (4,4), save_file= f"./images/{MODEL}_{args.model}_digit.png")
